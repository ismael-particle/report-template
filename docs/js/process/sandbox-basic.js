...
document.getElementById('organization_name').innerHTML = service_agreement_obj[0]["SANDBOX_EMAIL"]
document.getElementById('organization_service').innerHTML = 'Sandbox'

/* --------------> Printing Service agreement details table <--------------- */
data_service_details_arr = [
    ['Email', service_agreement_obj[0]["SANDBOX_EMAIL"]],  
    ['Sandbox ID', service_agreement_obj[0]["SANDBOX_ID"] ],
    ['Service agreement ID', service_agreement_obj[0]["SERVICE_AGREEMENT_ID"]],
    ['Metered device limit',
        (service_agreement_obj[0]["SANDBOX_METERED_DEVICES_LIMIT"]).toLocaleString("en-US")],
    ['Data operations limit',
        (service_agreement_obj[0]["SANDBOX_DATAOPS_LIMIT"]).toLocaleString("en-US")],
    ['Celular data limit in billing period [Mb]', 
        ( (service_agreement_obj[0]["SANDBOX_CELLULAR_DATA_LIMIT"])/(1000000) ).toLocaleString("en-US")],
...
if (iscellular_customer) {
	...
	let cellular_metered_devices_color = projection_clasification(
		...
		'QTY_METERED_DEVICES',
		service_agreement_obj[0]["SANDBOX_METERED_DEVICES_LIMIT"],
		[0.80,0.85,0.90]
		);
	...
	},
    {
		...
		chart_settings: {
			...
			y_label: 'Qty of metered devices',
			orientation: 'v',
			line_value: service_agreement_obj[0]["SANDBOX_METERED_DEVICES_LIMIT"],
			line_text_label: ('Device limit: ' + service_agreement_obj[0]["SANDBOX_METERED_DEVICES_LIMIT"] 
				+ ' devices' )
		}
	},
	{
	...
	},
    {
		...
		subtitle: "Cellular Data Average Consumption per Year per Device", //Section subtitle
		tooltip_content: "Contain all the available information about quantity of cellular data per month.",
		dowload_boolean: false, //true,false
		url_docs: "https://www.notion.so/particle/Consumption-activity-dashboard-3897d4e7a75346568440f7dde3764240",
		content_type: "simple-chart-bar", //simple-table,simple-chart,
		...
		chart_settings: {
			//COUNT_DEVICES,TIME_OFFLINE,STAGE,COLOR_STATE
			chart_data: cellular_data_monthly_average_ar,
			x_column: 'BILLING_PERIOD_START_DATE',
			y_column: 'MONTHLY_CELLULAR_DATA_QY_MB',
			colors: '', //If NULL this display blue
			x_label: 'Month',
...
/* ----------------------------> WiFi section <----------------------------- */
if (iswifi_customer) {
	//	->	Metered devices - WiFi - Sandbox-Basic
	//Set up color per percentage of metered devices in cellular devices
	let wifi_metered_devices_color = projection_clasification(
	...
	************************************************************/
	...
	},
	{
		...
		chart_settings: {
			//COUNT_DEVICES,TIME_OFFLINE,STAGE,COLOR_STATE
			chart_data: wifi_metered_devices_color,
			x_column: 'BILLING_USAGE_MONTH_START_DATE',
			y_column: 'QTY_METERED_DEVICES',
			colors: 'COLOR_STATE', //If NULL this display blue
			x_label: 'Month',
